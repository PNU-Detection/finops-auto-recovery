...
def _ensure_security_groups(vpc_id: str) -> tuple[str, str]:
    ...
    alb_sg = _find_or_create(
        "detection-traffic-alb-sg", "ALB SG for EDoS traffic trial"
    )
    inst_sg = _find_or_create(
        "detection-traffic-instance-sg", "Instance SG for EDoS traffic trial"
    )
...
def _ensure_launch_template(ec2, instance_sg: str) -> str:
    ...
    logger.info(
        "Launch Template 생성: %s (AMI: %s, user-data로 HTTP 서버 설치)",
...
def _ensure_alb(
    vpc_id: str, subnet_ids: list[str], alb_sg: str
) -> tuple[str, str, str]:
    ...
    if existing["LoadBalancers"]:
        ...
        lb = elb.describe_load_balancers(LoadBalancerArns=[lb["LoadBalancerArn"]])[
            "LoadBalancers"
        ][0]
...
def _ensure_target_group_and_listener(
    vpc_id: str, alb_arn: str, name: str, port: int
) -> tuple[str, str]:
    """(target_group_arn, tg_id_suffix) 반환. 리스너(외부 port)도 같이 만든다."""
    elb = boto3.client("elbv2", region_name=AWS_REGION)
    existing = (
        elb.describe_target_groups(Names=[name])
        if _tg_exists(elb, name)
        else {"TargetGroups": []}
    )
...
def _ensure_asg(
    autoscaling, name: str, lt_id: str, subnet_ids: list[str], tg_arn: str
) -> None:
...
def setup_all(n_anomaly: int, n_normal: int, n_edge: int = 0) -> None:
    ...
    for i in range(n_anomaly):
        ...
        tg_arn, _ = _ensure_target_group_and_listener(
            vpc_id, alb_arn, f"tg-anomaly-{i}", port
        )
    ...
    for i in range(n_edge):
        ...
        tg_arn, _ = _ensure_target_group_and_listener(
            vpc_id, alb_arn, f"tg-edge-{i}", port
        )
        _ensure_asg(autoscaling, name, lt_id, subnet_ids, tg_arn)
        resource_ports[name] = port
    for i in range(n_normal):
        ...
        tg_arn, _ = _ensure_target_group_and_listener(
            vpc_id, alb_arn, f"tg-normal-{i}", port
        )
        _ensure_asg(autoscaling, name, lt_id, subnet_ids, tg_arn)
        resource_ports[name] = port
    ...
    logger.info(
        "인스턴스가 헬스체크를 통과하려면(user-data 실행 시간 포함) 2~3분 정도 걸릴 수 있습니다."
    )
...
def teardown_all(n_anomaly: int, n_normal: int, n_edge: int = 0) -> None:
    ...
    try:
        ...
        if lbs:
            ...
            for listener in elb.describe_listeners(LoadBalancerArn=alb_arn)[
                "Listeners"
            ]:
...
def _traffic_loop(
    url: str, rps_getter, stop_event: threading.Event, label: str, rep: int
) -> None:
...
def _fetch_capacity_metrics(
    asg_name: str, end_time: datetime, n_points: int = 30, period_seconds: int = 300
) -> dict[str, list[float]]:
...
def detect(asg_name: str, alb_id_suffix: str, tg_id_suffix: str) -> dict:
    ...
    metrics["request_count"] = _fetch_alb_request_count(
        alb_id_suffix, tg_id_suffix, end_time
    )
    return detect_both("AutoScaling", asg_name, usage=metrics)


def run_full_pipeline_from_metrics(
    asg_name: str, raw_metrics: dict, bypass_approval: bool
) -> dict:
...
def run_anomaly_trial(
    ...
    traffic_thread = threading.Thread(
        ...
        args=(url, lambda: current_rps[0], stop_event, "anomaly", rep),
        daemon=True,
    )
    traffic_thread.start()
    logger.info(
        "[anomaly rep=%d, profile=%s] %s baseline rps=%.1f로 %d분 유지",
        rep,
        profile,
        asg_name,
        BASELINE_RPS,
    ...
    try:
        ...
        logger.info(
            "[anomaly rep=%d, profile=%s] before or_gate=%s",
            rep,
            profile,
        ...
        logger.info(
            "[anomaly rep=%d, profile=%s] rps=%.1f로 폭증, %d분 유지...",
            rep,
            profile,
            spike_rps,
            spike_minutes,
        ...
        logger.info(
            "[anomaly rep=%d] or_gate=%s (z=%s IF=%s) request_count 마지막=%s",
            rep,
            after["production"]["or_gate"],
            after["production"]["zscore_persistent"],
            after["production"]["iforest_triggered"],
            after["raw_metrics"]["request_count"][-3:],
        )

        pipeline_result = None
        if after["production"]["or_gate"]:
            pipeline_result = run_full_pipeline_from_metrics(
                asg_name, after["raw_metrics"], bypass_approval
            )
            logger.info(
                "[anomaly rep=%d] 파이프라인: anomaly_type=%s action=%s risk=%s approval_bypassed=%s "
                "apply_waf=%s action_result=%s qa_passed=%s",
                rep,
                pipeline_result.get("anomaly_type"),
                pipeline_result.get("selected_action"),
                pipeline_result.get("risk_level"),
                pipeline_result.get("approval_bypassed"),
                pipeline_result.get("apply_waf"),
                pipeline_result.get("action_result"),
...
def run_normal_trial(
    ...
    traffic_thread = threading.Thread(
        target=_traffic_loop,
        args=(url, lambda: BASELINE_RPS, stop_event, "normal", rep),
        daemon=True,
    )
    traffic_thread.start()
    ...
    try:
        ...
        before = detect(asg_name, alb_id_suffix, tg_id_suffix)
        logger.info(
            "[normal rep=%d] before or_gate=%s", rep, before["production"]["or_gate"]
        )
        ...
        after = detect(asg_name, alb_id_suffix, tg_id_suffix)
        logger.info(
            "[normal rep=%d] or_gate=%s (z=%s IF=%s)",
            rep,
            after["production"]["or_gate"],
            after["production"]["zscore_persistent"],
            after["production"]["iforest_triggered"],
        ...
        if after["production"]["or_gate"]:  # 오탐(FP)이면 실제로 뭘 했을지도 같이 기록
            ...
            logger.warning(
                "[normal rep=%d] 오탐 발생 - 파이프라인이 실제로 조치를 시도함: %s",
                rep,
                pipeline_result,
            )
...
def result_filename(n_normal: int, n_anomaly: int, n_edge: int = 0) -> Path:
    ...
    return (
        RESULT_DIR
        / f"autoscaling_edos_traffic_trial__{suffix}_scriptv{SCRIPT_VERSION}_{date_str}.json"
    )
...
def main() -> None:
    ...
    logger.info(
        "=== anomaly %d개 + edge %d개 + normal %d개, 총 %d개 시행 동시 병렬 시작 (트래픽 기반) ===",
        args.n_anomaly,
        args.n_edge,
        args.n_normal,
        args.n_anomaly + args.n_edge + args.n_normal,
    )
    ...
    for name, m in metrics.items():
        ...
        logger.info(
            "[%s] TP=%d TN=%d FP=%d FN=%d / accuracy=%s recall=%s",
            name,
            c["TP"],
            c["TN"],
            c["FP"],
    ...
    if args.n_edge > 0:
        for profile_name in ("extreme_spike", "edge_spike"):
            subset = [
                r
                for r in results
                if r["label"] == "anomaly" and r.get("profile") == profile_name
            ] + normal_results
            if any(r["label"] == "anomaly" for r in subset):
                ...
                logger.info(
                    "[profile=%s] production recall=%s (n_anomaly=%d)",
                    profile_name,
                    f"{pm['recall']:.1%}" if pm["recall"] is not None else "N/A",
                    pm["n_anomaly"],
                )
        ...
        json.dump(
            {
                ...
                "params": {
                    ...
                    "spike_minutes": args.spike_minutes,
                    "baseline_rps": BASELINE_RPS,
                    "spike_rps": SPIKE_RPS,
                    "edge_spike_rps": EDGE_SPIKE_RPS,
    ...
    logger.warning(
        "테스트 후 반드시 정리하세요: --teardown --n-anomaly %d --n-normal %d --n-edge %d",
        args.n_anomaly,
        args.n_normal,
        args.n_edge,
    )
