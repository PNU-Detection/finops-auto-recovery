...
if hasattr(sys.stdout, "reconfigure"):
    sys.stdout.reconfigure(
        encoding="utf-8"
    )  # Windows 콘솔 기본 cp949가 em-dash 등에서 죽는 문제 방지
...
def check_capacity_budget(
    ...
    current_instances = sum(
        ...
        if not any(
            t.get("Key") == "Purpose" and t.get("Value") == "detection-trial"
    ...
    peak_instances = (
        current_instances
        + n_normal * NORMAL_CAPACITY_HIGH
        + n_anomaly * anomaly_capacity
    )
    peak_vcpu = peak_instances * VCPU_PER_INSTANCE
    peak_ebs = peak_instances * EBS_GB_PER_INSTANCE
    instance_hours = (
        n_normal * NORMAL_CAPACITY_HIGH + n_anomaly * anomaly_capacity
    ) * hours_estimate

    logger.info(
        "예상 최대 동시 인스턴스 %d대 (기존 %d + 이번 %d)",
        peak_instances,
        current_instances,
        peak_instances - current_instances,
    )
    ...
    logger.info(
        ...
        FREE_TIER_EBS_GB,
        "-> 초과분은 시간비례 과금"
        if peak_ebs > FREE_TIER_EBS_GB
        else "-> 프리티어 내",
    )
    logger.info(
        "  인스턴스-시간: 약 %.1f (프리티어 월 %d시간)",
        instance_hours,
        FREE_TIER_INSTANCE_HOURS,
    )
    logger.warning(
        "프리티어 EC2/EBS는 계정 생성 후 12개월만 적용됨 - 12개월 초과 계정이면 "
        "위 한도가 없고 전부 과금됨. Billing 콘솔에서 확인할 것."
    )

    if peak_vcpu > vcpu_limit:
        msg = (
            f"예상 최대 vCPU {peak_vcpu}가 쿼터 {vcpu_limit}를 초과합니다. "
            f"--anomaly-capacity 축소, --n-anomaly/--n-normal 축소, 기존 인스턴스 정리, "
            f"또는 쿼터 증설 중 하나가 필요합니다."
        )
        if skip_quota_check:
            logger.warning(
                "%s (--skip-quota-check 지정으로 계속 진행 - launch 실패 시 "
                "desired만 오르고 in_service는 안 오르는 왜곡된 측정이 될 수 있음)",
                msg,
            )
        else:
...
def _ensure_asg(
    autoscaling, name: str, lt_id: str, subnet_ids: str, desired: int, max_size: int
) -> None:
...
def setup_asgs(
    n_anomaly: int, n_normal: int, anomaly_capacity: int
    ...
    for name in normal_names:
        _ensure_asg(
            ...
            subnet_ids,
            NORMAL_CAPACITY_LOW,
            max_size=NORMAL_CAPACITY_HIGH,
        )

    logger.info(
        "ASG 준비 완료: anomaly=%d개, normal=%d개 (모두 초기 capacity=%d, jitter로 %d~%d 사이 변동 예정)",
        n_anomaly,
        n_normal,
        NORMAL_CAPACITY_LOW,
        NORMAL_CAPACITY_LOW,
        NORMAL_CAPACITY_HIGH,
    )
    logger.warning(
        "생성 직후에는 CloudWatch 창(30포인트=2.5시간)의 앞부분이 0으로 채워진다. "
...
def _jitter_loop(
    asg_name: str, stop_event: threading.Event, label: str, rep: int
) -> None:
    ...
    while not stop_event.wait(JITTER_INTERVAL_SEC):
        current = (
            NORMAL_CAPACITY_HIGH
            if current == NORMAL_CAPACITY_LOW
            else NORMAL_CAPACITY_LOW
        )
        try:
            autoscaling.set_desired_capacity(
                AutoScalingGroupName=asg_name, DesiredCapacity=current
            )
            logger.info(
                "[%s rep=%d] jitter: %s capacity -> %d", label, rep, asg_name, current
            )
        except Exception as exc:
...
def run_anomaly_trial(
    ...
    spiked = False
    logger.info(
        "[anomaly rep=%d] asg=%s 베이스라인 %d분(jitter %d<->%d 적용) 유지 후 capacity ->%d",
        rep,
        asg_name,
        baseline_minutes,
        NORMAL_CAPACITY_LOW,
    ...
    jitter_thread = threading.Thread(
        target=_jitter_loop,
        args=(asg_name, jitter_stop, "anomaly-baseline", rep),
        daemon=True,
    )
    jitter_thread.start()
    try:
        ...
        logger.info(
            "[anomaly rep=%d] before production.or_gate=%s",
            rep,
            before["production"]["or_gate"],
        )

        autoscaling.set_desired_capacity(
            AutoScalingGroupName=asg_name, DesiredCapacity=anomaly_capacity
        )
        spiked = True
        logger.info(
            "[anomaly rep=%d] capacity=%d 설정, 인스턴스 기동 대기 %d초...",
            rep,
            anomaly_capacity,
            startup_wait_sec,
        )
        ...
        if not spike_realized:
            logger.warning(
                "[anomaly rep=%d] in_service=%d로 목표 %d 미달 - launch가 실패했을 수 있음"
        ...
        # 확실히 여러 포인트가 쌓이게 한다.
        logger.info(
            "[anomaly rep=%d] 지속성 확보를 위해 %d분 추가 유지...",
            rep,
            sustain_minutes,
        )
        ...
        logger.info(
            "[anomaly rep=%d] production.or_gate=%s (z=%s, IF=%s) / teammate=%s",
            rep,
            after["production"]["or_gate"],
            after["production"]["zscore_persistent"],
        ...
        return {
            ...
            "resource": asg_name,
            "label": "anomaly",
            "capacity_before": NORMAL_CAPACITY_LOW,
            "capacity_after": anomaly_capacity,
            "in_service_after_spike": actual_in_service,
            "spike_realized": spike_realized,
            "before": before,
            "after": after,
        ...
        return {
            ...
            "label": "anomaly",
            "error": str(exc),
            "detected_production": None,
            "detected_teammate_compat": None,
            "detected_iforest_only": None,
            "detected_zscore_only": None,
        }
    finally:
        ...
        if spiked:
            try:
                autoscaling.set_desired_capacity(
                    AutoScalingGroupName=asg_name, DesiredCapacity=NORMAL_CAPACITY_LOW
                )
                logger.info(
                    "[anomaly rep=%d] capacity=%d로 복구", rep, NORMAL_CAPACITY_LOW
                )
            except Exception as exc:
                logger.error(
                    "[anomaly rep=%d] capacity 복구 실패(수동 확인 필요): %s", rep, exc
                )
...
def run_normal_trial(
    ...
    t0 = time.time()
    logger.info(
        "[normal rep=%d] asg=%s capacity %d<->%d jitter 유지 (총 %d분 후 측정)",
        rep,
        asg_name,
        NORMAL_CAPACITY_LOW,
        NORMAL_CAPACITY_HIGH,
    ...
    try:
        ...
        before = detect("AutoScaling", asg_name)
        logger.info(
            "[normal rep=%d] before production.or_gate=%s",
            rep,
            before["production"]["or_gate"],
        )
        ...
        after = detect("AutoScaling", asg_name)
        logger.info(
            "[normal rep=%d] production.or_gate=%s (z=%s, IF=%s) / teammate=%s",
            rep,
            after["production"]["or_gate"],
            after["production"]["zscore_persistent"],
            after["production"]["iforest_triggered"],
...
def result_filename(
    anomaly_capacity: int, wait_sec: int, n_normal: int, n_anomaly: int
) -> Path:
    date_str = datetime.now().strftime("%Y%m%d")
    name = (
        f"autoscaling_edos_trial__capacity{anomaly_capacity}_wait{wait_sec}s_"
...
def main() -> None:
    ...
    parser.add_argument(
        "--setup", action="store_true", help="ASG 생성 + group metrics 활성화"
    )
    ...
    parser.add_argument(
        "--anomaly-capacity",
        type=int,
        default=DEFAULT_ANOMALY_CAPACITY,
    ...
    parser.add_argument(
        "--startup-wait-sec", type=int, default=600, help="인스턴스 기동 대기(초)"
    )
    parser.add_argument(
        "--metric-wait-sec", type=int, default=300, help="CloudWatch 반영 대기(초)"
    )
    parser.add_argument(
        "--sustain-minutes",
        type=int,
    ...
    parser.add_argument(
        "--vcpu-limit", type=int, default=32, help="계정 vCPU 쿼터(L-1216C47A)"
    )
    parser.add_argument(
        "--skip-quota-check",
        action="store_true",
    ...
    if args.baseline_minutes < 150:
        logger.warning(
            "--baseline-minutes=%d 는 창 전체(150분)보다 짧습니다. 창 앞부분이 0으로 "
    ...
    hours_estimate = (
        args.baseline_minutes * 60
        + args.startup_wait_sec
        + args.sustain_minutes * 60
        + args.metric_wait_sec
    ) / 3600

    if args.setup:
        check_capacity_budget(
            args.n_anomaly,
            args.n_normal,
            args.anomaly_capacity,
            args.vcpu_limit,
            hours_estimate,
    ...
    metrics = {
        "production(detection_node 실제 방식)": compute_metrics(
            results, "detected_production"
        ),
        "iforest_only(persistence 적용)": compute_metrics(
            results, "detected_iforest_only"
        ),
        "zscore_only(persistence 적용)": compute_metrics(
            results, "detected_zscore_only"
        ),
        "teammate_compat(v1까지의 방식)": compute_metrics(
            results, "detected_teammate_compat"
        ),
    }
    for name, m in metrics.items():
        c = m["confusion_matrix"]
        logger.info(
            "[%s] TP=%d TN=%d FP=%d FN=%d / accuracy=%s recall=%s FPR=%s",
            name,
            c["TP"],
            c["TN"],
            c["FP"],
        ...
        if m["recall_ci_95_clopper_pearson"]:
            ...
            logger.info(
                "    recall 95%% CI(Clopper-Pearson) = [%.1f%%, %.1f%%]",
                lo * 100,
                hi * 100,
            )
    ...
    unrealized = [
        r
        for r in results
        if r.get("label") == "anomaly" and r.get("spike_realized") is False
    ]
    if unrealized:
        logger.warning(
            "스파이크가 목표 용량에 도달하지 못한 anomaly 시행 %d건 - 해당 시행은 "
            "'desired만 급증, in_service 평탄'이라 의도한 패턴과 다름: %s",
            len(unrealized),
            [r["resource"] for r in unrealized],
        )

    out_path = result_filename(
        args.anomaly_capacity, args.metric_wait_sec, args.n_normal, args.n_anomaly
    )
    ...
    logger.warning(
        "테스트 후 반드시 정리하세요: --teardown --n-anomaly %d --n-normal %d",
        args.n_anomaly,
        args.n_normal,
    )
