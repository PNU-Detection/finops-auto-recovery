...
def verify(manifest_path: str) -> None:
    ...
    for inst in manifest["instances"]:
        ...
        print(f"  [{group} {inst['instance_id']}] 최근 CPU 포인트: {[round(v,2) for v in values]}")
...
def resend_load(manifest_path: str, groups: list[str]) -> None:
    ...
    remaining_sec = int((datetime.fromisoformat(remaining) - datetime.now(timezone.utc)).total_seconds())
    ...
    for inst in manifest["instances"]:
        ...
        ssm.send_command(InstanceIds=[inst["instance_id"]], DocumentName="AWS-RunShellScript", Parameters={"commands": cmds})
        print(f"[재전송][{inst['group']} {inst['instance_id']}] 목표 {target}% ({remaining_sec}초 남음)")
...
def main() -> None:
    ...
    parser.add_argument("--groups", type=str, help="--resend-load 대상 그룹, 콤마구분(기본: 전체)")
    ...
    if args.setup:
        ...
        parser.error("--setup, --verify, --resend-load, --teardown 중 하나를 지정하세요")
