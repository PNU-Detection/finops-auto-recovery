...
FUNCTION_SETS = {
    "own": (
        ["detection-test-lambda"]
        + [f"detection-test-lambda-anomaly-{i}" for i in range(2, 6)],
...
def detect(
    resource_type: str, resource_id: str, n_points: int = 30, period_seconds: int = 300
) -> dict:
    """실제 프로덕션 detection_node() 그대로 호출 + 원본 raw_metrics 전체를 함께 반환."""
    assembled = assemble_resource(
        resource_id, resource_type, n_points=n_points, period_seconds=period_seconds
    )
...
def _assert_same_account(invoke_session: boto3.Session | None) -> tuple[str, str]:
    ...
    if detect_acct != invoke_acct:
        logger.error(
            "탐지 계정(%s)과 트래픽 생성 계정(%s)이 다름 — 호출 대상과 관측 대상이 "
            "달라서 측정이 성립하지 않는다. --invoke-profile 을 맞추거나 .env의 "
            "AWS_ACCESS_KEY_ID/AWS_PROFILE 중 하나를 정리할 것.",
            detect_acct,
            invoke_acct,
        )
        sys.exit(1)
...
def _run_trial(
    ...
    base_rate = (
        min(0.98, max(0.0, _jitter(baseline_error_rate)))
        if label == "anomaly"
        else spike_rate
    )
    ...
    if label == "anomaly":
        actual_interval = min(
            actual_interval, max(2, period_seconds // MIN_CALLS_PER_SPIKE_PERIOD)
        )
    calls_per_period = max(1, period_seconds // actual_interval)

    logger.info(
        "[%s %s] 시작 (구간 %d개 x %ds = %.1f시간, 구간당 %d회, 평상시 에러율 %.2f, "
        "마지막 %d개 구간 에러율 %.2f(기준 %.2f), 간격 %ds(기준 %ds))",
        label,
        function_name,
        n_points,
        period_seconds,
        n_points * period_seconds / 3600,
        calls_per_period,
        base_rate,
        spike_periods if label == "anomaly" else 0,
        spike_rate,
        error_rate,
        actual_interval,
        interval_sec,
    ...
    try:
        ...
        logger.info(
            "[%s %s] before anomaly_flag=%s (z=%s, IF=%s)",
            label,
            function_name,
            before["anomaly_flag"],
            before["anomaly_score_zscore"],
        ...
        for period_idx in range(n_points):
            ...
            period_stats.append(
                {
                    "period": period_idx,
                    "spike": is_spike,
                    "n_calls": calls_per_period,
                    "n_forced_errors": n_err,
                }
            )
            ...
            logger.debug(
                "[%s %s] 구간 %d/%d(%s): %d회(에러 %d) 완료 (%.1fs), %.1fs 대기",
                label,
                function_name,
                period_idx + 1,
                n_points,
                "SPIKE" if is_spike else "정상",
                calls_per_period,
                n_err,
                elapsed,
                max(0.0, remaining),
        ...
        logger.info(
            "[%s] 호출 완료 (강제 에러 %d/%d, invoke 실패 %d), CloudWatch 반영 %d초 대기...",
            function_name,
            n_err_total,
            n_calls_total,
            n_invoke_failed,
        ...
        logger.info(
            "[%s %s] anomaly_flag=%s (z=%s, IF=%s, triggered=%s), %.1f분",
            label,
            function_name,
            after["anomaly_flag"],
...
def result_filename(
    ...
    name = (
        f"lambda_retry_repeated_trial__window{hours:.1f}h_spike{spike_periods}p_"
...
def main() -> None:
    ...
    parser.add_argument(
        "--period-seconds", type=int, default=300, help="구간 길이 (기본 300초 = 5분)"
    )
    parser.add_argument(
        "--spike-periods",
        type=int,
    ...
    invoke_session = (
        None
        if args.invoke_profile == "env"
        else boto3.Session(profile_name=args.invoke_profile)
    )
    ...
    for fn_name, label, _, _ in functions:
        try:
            ...
            logger.error(
                "--invoke-profile 로 권한 있는 프로필을 지정하거나 IAM 정책에 "
                "lambda:InvokeFunction 을 추가한 뒤 다시 실행하세요."
            )
            sys.exit(1)
        if not resp.get("FunctionError"):
            logger.error(
                "사전 점검 실패 — '%s' 가 에러 payload(%s)에도 정상 응답함. "
                "핸들러가 보는 키가 다른 것 같다(ERROR_PAYLOAD 주석 참고). "
                "이대로 돌리면 Errors 지표가 0이라 전 시행이 FN으로 나온다.",
                fn_name,
                ERROR_PAYLOAD.decode(),
            )
            sys.exit(1)
    logger.info(
        "사전 점검 통과: 계정 %s, 함수 %d개 전부 invoke 가능 + 에러 payload 동작 확인",
        account_id,
        len(functions),
    )
