...
def _run_trial(
    ...
    actual_rate = (
        min(
            0.98,
            max(0.0, error_rate * random.uniform(1 - jitter_ratio, 1 + jitter_ratio)),
        )
        if error_rate > 0
        else 0.0
    )
    actual_interval = max(
        5, round(interval_sec * random.uniform(1 - jitter_ratio, 1 + jitter_ratio))
    )
    n_calls = int(phase_minutes * 60 / actual_interval)

    logger.info(
        "[%s %s] 시작 (에러율=%.2f(기준 %.2f), 간격=%ds(기준 %ds), 호출 %d회)",
        label,
        function_name,
        actual_rate,
        error_rate,
    ...
    try:
        ...
        logger.info(
            "[%s] 호출 완료 (에러 %d/%d), CloudWatch 반영 %d초 대기...",
            function_name,
            n_err,
            n_calls,
            wait_sec,
        ...
        logger.info(
            "[%s %s] anomaly_flag=%s (z=%s, IF=%s, triggered=%s), %.1fs",
            label,
            function_name,
            after["anomaly_flag"],
...
def result_filename(phase_minutes: int, n_normal: int, n_anomaly: int) -> Path:
    ...
    name = (
        f"lambda_retry_repeated_trial__phase{phase_minutes}m_"
...
def main() -> None:
    ...
    try:
        probe = boto3.Session(profile_name=args.invoke_profile).client(
            "lambda", region_name=AWS_REGION
        )
        probe.invoke(
            FunctionName=LAMBDA_FUNCTIONS[0][0], InvocationType="Event", Payload=b"{}"
        )
        logger.info("사전 점검 통과: 프로필 '%s'로 invoke 가능", args.invoke_profile)
    except Exception as exc:
        logger.error(
            "사전 점검 실패 — 프로필 '%s'로 invoke 불가: %s", args.invoke_profile, exc
        )
        logger.error(
