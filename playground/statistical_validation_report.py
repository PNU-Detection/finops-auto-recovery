...
SCENARIOS = [
    ...
    {
        # [버그 수정, 2026-09-14] 기존에 "ec2_repeated_trial__n8-5_scriptv1_20260912.json"을
        # 가리켰으나, 이 파일은 profile이 전부 target_cpu_12.0pct/45.0pct로 되어 있는
        # EC2 오버프로비저닝 재측정 데이터였다(같은 인스턴스ID·raw_metrics를
        # ec2_overprovision_repeated_trial__converted.json과 공유 — 실측으로 확인).
        # 진짜 좀비(완전 유휴) 데이터는 이 파일이 아니라 아래 replay 파일이다.
        "name": "EC2 좀비(사용 안 되는 유휴 인스턴스)",
        "file": "ec2_zombie_replay_trial__n8-5_scriptv1_20260910.json",
        "format": "split",
        "methods": {"detected": "production"},
    },
    {
...
def main() -> None:
    ...
    # ── 파이프라인 전체 성공률: 시나리오별 production 방식 confusion matrix를
    # 전부 더해서(풀링) 하나의 통합 accuracy/recall/precision/F1로 계산한다.
    # (비율을 평균내는 게 아니라 raw TP/FN/FP/TN을 합산 후 재계산 — 풀링 원칙) ──
    ...
    pooled_n_anomaly = pooled_tp + pooled_fn
    pooled_n_normal = pooled_fp + pooled_tn
    pooled_total = pooled_n_anomaly + pooled_n_normal
    pooled_accuracy = (pooled_tp + pooled_tn) / pooled_total if pooled_total else None
    pooled_recall = pooled_tp / pooled_n_anomaly if pooled_n_anomaly else None
    ...
    pooled_f1 = (
        ...
        if (pooled_precision and pooled_recall and (pooled_precision + pooled_recall) > 0)
        else None
    )
    pooled_fpr = pooled_fp / pooled_n_normal if pooled_n_normal else None

    report["pipeline_overall_pooled"] = {
        "scenarios_included": pooled_scenarios,
        "confusion_matrix": {"TP": pooled_tp, "FN": pooled_fn, "FP": pooled_fp, "TN": pooled_tn},
        "n_anomaly": pooled_n_anomaly,
        "n_normal": pooled_n_normal,
        "accuracy": pooled_accuracy,
        "accuracy_ci_95": clopper_pearson_ci(pooled_tp + pooled_tn, pooled_total),
        "recall": pooled_recall,
        "recall_ci_95": clopper_pearson_ci(pooled_tp, pooled_n_anomaly),
        "precision": pooled_precision,
        "f1_score": pooled_f1,
        "false_positive_rate": pooled_fpr,
        "fpr_ci_95": clopper_pearson_ci(pooled_fp, pooled_n_normal),
    }
    print(f"\n=== 파이프라인 전체 성공률(풀링, {pooled_scenarios}) ===")
    print(f"  n_anomaly={pooled_n_anomaly} n_normal={pooled_n_normal} n_total={pooled_total} "
          f"(TP={pooled_tp} FN={pooled_fn} FP={pooled_fp} TN={pooled_tn})")
    if pooled_accuracy is not None:
        print(f"  accuracy={pooled_accuracy*100:.1f}% CI={report['pipeline_overall_pooled']['accuracy_ci_95']}")
    if pooled_recall is not None:
        print(f"  recall={pooled_recall*100:.1f}% CI={report['pipeline_overall_pooled']['recall_ci_95']}")
